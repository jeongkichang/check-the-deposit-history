...
// 작업 중인 상태는 계속 작업 중 상태로 남아있는건지, 아니면 진짜로 해결 중이기 때문에 작업 중인 상태인 건지를 어떻게 구별할 수 있을까
// 표준 출력? 표준 에러..?

// 이전 회차로 돌아갈게 아니라, 그냥 처리되지 않은 회차를 모두 조회해서 처리할 수 있도록 하면 어떨까

// 계정별로, access_token 추려가지고 그 계정에 대한 권한으로 각각 gmail api를 호출해야한다.
// 그럴려면,, gmail api 호출이 필요할 때, 누구의 계정에 의한 것인지를 체크해야한다.

// 각각 계정마다, 탈퇴하기 전까지 끝까지 수집할 수 있는 구조를 만들어야 한다.
// expire_time이 도래되면 미리 알림이 가도록 해야한다.
// 알람은 일단 mail gun 서비스를 이용하든지 해야할 거 같다.
// PWA 이면 푸시 알람도 줄 수 있을 것이다.

// 진득하게 할 수 있는 무언가여야 한다.
// 오늘은 아쉽게도 진득하게 할 수 없으니 하지 않는다.

// 오늘은 가족행사가 있다. 역시 시간이 빠듯할 거 같다.
